...
int PAnonymizer::anonymizev6( in6_addr* ip){
   ...
   // UINT32 first4bytes_pad, first4bytes_input;
	...
	rin_input[pos/8] |= ((m_pad[pos/8] << (pos%8)) & 0xff) >> (pos%8);
	 ...
	 *  one-time-pad
	 */
	pad.s6_addr[pos/8] |=  (rin_output[0] >> 7) << (7-(pos%8));
    }
    /* XOR the orginal address with the pseudorandom one-time-pad */
    for(i=0;i<16;i++) {
    	ip->s6_addr[i] = ip->s6_addr[i] ^ pad.s6_addr[i];
    }
    return 0;
}
